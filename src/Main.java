...
//    }
    Lexer lexer = new Lexer("let b = 10 + 2;");
    ...
    HIRGen hirGenerator = new HIRGen(g.statements, g.symbolTable, parser.idx);
    ...
    IO.println("- LIR:");
    String lir = lirGenerator.generate();
    IO.println(lir);
    LIRStorer lirStorer = new LIRStorer(Paths.get(args[0]));
    lirStorer.store(lir);
    Vm vm = new Vm(lir);
    vm.execute();
